...
public class RouteResource {
    ...
    private final SpeedLimitAwareRequestTransformer speedLimitRequestTransformer;
    ...
    public RouteResource(GraphHopperConfig config, GraphHopper graphHopper, ProfileResolver profileResolver, 
                         GHRequestTransformer ghRequestTransformer, TrafficAwareRequestTransformer trafficRequestTransformer,
                         SpeedLimitAwareRequestTransformer speedLimitRequestTransformer,
        ...
        this.speedLimitRequestTransformer = speedLimitRequestTransformer;
    ...
    public Response doGet(
            ...
            @QueryParam("avoid_eds_roads") @DefaultValue("false") boolean avoidEdsRoads,
            @QueryParam("avoid_custom_areas") @DefaultValue("false") boolean avoidCustomAreas,
            @QueryParam("car_type_id") Integer carTypeId,
            @QueryParam("enable_speed_limits") @DefaultValue("true") boolean enableSpeedLimits) {
        ...
        // 3+ point'li route'larda alternatif route'ları devre dışı bırak
        if (points.size() > 2) {
            logger.info("3+ point route detected ({} points), disabling alternative routes", points.size());
            request.getHints().putObject("alternative_route.max_paths", 0);
            request.getHints().putObject("alternative_route.max_weight_factor", 0);
            request.getHints().putObject("alternative_route.max_share_factor", 0);
        }
        ...
        request.setPoints(points).
                ...
                putObject("avoid_eds_roads", avoidEdsRoads).
                putObject("avoid_custom_areas", avoidCustomAreas).
                putObject("car_type_id", carTypeId).
                putObject("enable_speed_limits", enableSpeedLimits);
        ...
        // Apply traffic-aware routing transformation
        request = trafficRequestTransformer.transformRequest(request);
        
        // Apply speed limit aware routing transformation
        request = speedLimitRequestTransformer.transformRequest(request);
