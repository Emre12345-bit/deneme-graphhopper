...
public class TrafficAwareRequestTransformer {
    ...
    public GHRequest transformRequest(GHRequest request) {
        ...
        // Check if profile is foot or bike - always apply alternative route parameters and skip traffic-aware routing
        String profileName = request.getProfile();
        if (profileName != null && (profileName.equals("foot") || profileName.equals("bike"))) {
            logger.info("Skipping traffic-aware routing for {} profile - not applicable for pedestrians/cyclists", profileName);
            
            // Always apply alternative route parameters for foot/bike profiles (regardless of URL parameters)
            logger.info("🚶‍♂️🚴‍♂️ FOOT/BIKE ALTERNATIVE ROUTES: Always applying alternative route parameters for {} profile", profileName);
            
            // Clone the request to apply alternative route parameters
            ...
            // Configure alternative routes for foot/bike profiles
            ...
            logger.info("🚶‍♂️🚴‍♂️ FOOT/BIKE ALTERNATIVE ROUTES: Applied alternative route parameters for {} profile", profileName);
        ...
        logger.info("Starting traffic-aware request transformation for profile: {} (avoid_eds_roads={}, avoid_custom_areas={})", 
                   profileName, avoidEdsRoads, avoidCustomAreas);
        ...
        try {
            ...
            // Get the profile for this request (profileName already defined above)
