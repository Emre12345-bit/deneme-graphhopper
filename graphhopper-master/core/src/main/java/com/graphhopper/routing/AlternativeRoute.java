...
public class AlternativeRoute extends AStarBidirection implements RoutingAlgorithm {
    ...
    public AlternativeRoute(Graph graph, Weighting weighting, TraversalMode traversalMode, PMap hints) {
        ...
        int maxPathsFromHints = hints.getInt(MAX_PATHS, 2);
        // 3+ point'li route'larda alternatif route'ları devre dışı bırak
        if (maxPathsFromHints < 2) {
            // Normal routing algoritması kullan
            this.maxPaths = 1; // Sadece en iyi route'u döndür
        } else {
            this.maxPaths = maxPathsFromHints;
        }
