...
public class Router {
    ...
    protected GHResponse routeAlt(GHRequest request, Solver solver) {
        // 3+ point'li route'larda alternatif route'ları devre dışı bırak ve normal via routing kullan
        if (request.getPoints().size() > 2) {
            // Alternatif route parametrelerini temizle ve normal via routing'e yönlendir
